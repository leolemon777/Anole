...
try {
    & $Cargo test -p anole-core 'preset::tests'
    if ($LASTEXITCODE -ne 0) { throw 'core preset contract tests failed' }

    & $Cargo test -p anole-core 'rust_preset_library_matches_public_schema'
    if ($LASTEXITCODE -ne 0) { throw 'preset JSON Schema contract test failed' }

    & $Cargo test -p anole-desktop 'preset_library_write_is_recoverable_from_backup'
