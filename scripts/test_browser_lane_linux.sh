...
cd "${FW_SRC:-/home/leo/linux-runs/Anole/src}"
cargo build -p anole-cli 2>&1 | tail -1
FX=/home/leo/linux-runs/Anole/fixtures
rm -f /tmp/svg-linux.pdf
./target/debug/anole convert $FX/sample.svg --to pdf --output /tmp/svg-linux.pdf 2>&1 | tail -2
"$HOME/miniforge/envs/ocr/bin/pdftotext" /tmp/svg-linux.pdf - 2>/dev/null | grep -m1 "SVG MATRIX"
rm -f /tmp/html-linux.pdf
./target/debug/anole convert $FX/sample.html --to pdf --output /tmp/html-linux.pdf 2>&1 | tail -2
