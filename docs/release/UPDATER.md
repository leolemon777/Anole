...
- `apps/desktop/src-tauri/tauri.conf.json` enables `bundle.createUpdaterArtifacts`,
  ...
  `https://github.com/leolemon777/Anole/releases/latest/download/latest.json`.
- The **release keypair** (rotated 2026-09-01 from the dev pair, strong random
  password) lives at `target/updater-keys/anole-release.key` (private)
  ...
  dev pair (`anole-updater.key`, empty password) is retained only for
