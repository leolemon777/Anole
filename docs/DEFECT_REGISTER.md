...
|---|---|---|---|---|---|
| R-011 | P2 | Fixed | The WebView accessibility audit script no longer matches the shipped UI: `scripts/cdp_accessibility_audit.mjs` waits for `.shell`, `header nav`, `#input-path`, and `.skip-link`, none of which exist since the Meadowlark/chicago95 UI rework (5f1857c); the run times out at `waitFor FormatWright document` before any assertion, so the "automated accessibility baseline green" claim in MASTER §1.1 is stale for the current DOM | Reproduced 2026-09-07: `test_desktop_accessibility.ps1` times out at `waitFor`; CDP probe of a running debug build shows the live document (title "Anole", `.drop-zone`/`.settings-grid` present) but no `.shell`/`#input-path` nodes; `git log` shows the audit last touched before/at the UI rework | Audit adapted to the chicago95 DOM (`.fw-main-window` readiness, `nav.fw-tabs-nav` landmark, tab `[aria-current]`, DPR float tolerance, 45s cold-start allowance) and the app regained the three landmarks the UI rework had dropped: `nav.fw-tabs-nav` with localized label, `aria-current="page"` on the active tab, and a `main.fw-tabs-main` wrapper. 2026-09-07 rerun green through the real WebView2 tree: 210 nodes, 0 unnamed focusable controls, zh-CN→en switch, skip-link→#main-content keyboard flow, 200% viewport overflow-free, reduced-motion + forced-colors honored (`.artifacts/desktop-accessibility/suite-1a9ec04c…`). Remains Fixed until a CI-hosted rerun |
