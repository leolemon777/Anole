...
The Tauri 2 desktop application now invokes the same `anole-core::prepare_conversion` path intended for every first-party surface. The React shell exposes Convert, Jobs, Engines, Reports, and Settings rather than only the earlier queue-projection architecture spike.
...
- `anole-desktop.exe` starts a native window titled `Anole`, remains responsive, and is then terminated by the harness.
