...
- `uninstall.exe /S` removes the install root, both app-state roots, and Anole's owned shell keys (`Anole` Open-in plus the 17 generated Convert verbs). It must **not** look for the obsolete `AnoleConvert` name.
